...
- `MC_API_KEY` — bearer token. Either extracted from MC's auto-generated `.env` during install, or generated by `setup-openclaw.sh` (in which case the same value must be set on MC's side too).
...
4. **Outbound auth — agent posts to MC with bearer token.** Use `MC_URL` + `MC_API_KEY` from env. Never hardcode either.
5. **TODO_VERIFY on first install:**
   - Confirm MC's `.env` variable name for the bearer token (script tries common names: `API_KEY`, `ADMIN_TOKEN`, etc.)
   - Confirm the agent → MC transport (REST POST? WebSocket? GraphQL?) and which endpoint expects the bearer token
   - When exposing via tailscale: just edit `MC_URL` in `.env.local` to the tailscale address. No other changes.
